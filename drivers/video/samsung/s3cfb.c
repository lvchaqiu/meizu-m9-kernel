...
#include "m9w_logo.h"
	...
	for (i = Y_MEIZUM9_LOGO_START; i < Y_MEIZUM9_LOGO_START + MEIZUM9_LOGO_HEIGHT; i++) {
		for (j = X_MEIZUM9_LOGO_START; j < X_MEIZUM9_LOGO_START + MEIZUM9_LOGO_WIDTH; j++) {
...
#endif	/* CONFIG_FB_S3C_SPLASH_SCREEN */
...
static void s3cfb_late_resume(struct early_suspend *h)
{
	...
	pr_debug("s3cfb_late_resume is complete\n");
	return ;
}
#else
static void s3cfb_early_suspend(struct early_suspend *h)
