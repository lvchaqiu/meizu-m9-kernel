...
extern ssize_t wifi_card_state_show(struct device*, struct device_attribute *, char *);
extern ssize_t wifi_card_state_store(struct device *, struct device_attribute *, const char *, size_t);

static DEVICE_ATTR(wifi_card_state, S_IRUGO|S_IWUSR|S_IWGRP,
  		 				wifi_card_state_show, wifi_card_state_store);
...
static int wifi_probe(struct platform_device *pdev)
{
	...
	return sysfs_create_file(&pdev->dev.kobj, &dev_attr_wifi_card_state.attr);
}
...
static int wifi_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_file(&pdev->dev.kobj, &dev_attr_wifi_card_state.attr);
	return 0;
}
...
typedef struct dhd_info {
	...
	struct notifier_block dhd_pm_notifier;
	int power_state;
} dhd_info_t;
...
{
	dhd_info_t *dhd = container_of(nfb, dhd_info_t, dhd_pm_notifier);
	int ret = NOTIFY_DONE;

	switch (action) {
	case PM_HIBERNATION_PREPARE:
	case PM_SUSPEND_PREPARE:
#ifdef CONFIG_BCM4329_SUSPEND_POWER
		if (!IS_ERR_OR_NULL(dhd)) {
			dhd_pub_t *dhdp = &dhd->pub;
			if (dhdp && dhdp->up) {
				if (dhd->power_state != PM_MAX) {
					int power_mode = PM_MAX;
					ret = dhdcdc_set_ioctl(dhdp, 0, WLC_SET_PM,
								(char *)&power_mode, sizeof(power_mode));
					if (unlikely(ret < 0))
						pr_err("%s: dhdcdc_set_ioctl failed\n", __func__);
					else {
						dhd->power_state = power_mode;
						/* Maybe need to wait about 20 ms for wifi lock release*/
						msleep_interruptible(20);
					}
				}
			} else {
				pr_err("%s: dhdp is not upn", __func__);
			}
		}
#endif
	...
	if (dhd && dhd->up) {
		if (value && dhd->in_suspend) {
...
#ifndef CONFIG_BCM4329_SUSPEND_POWER
			ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_PM,
				(char *)&power_mode, sizeof(power_mode));
			if (ret >= 0)
				dhd->info->power_state = power_mode;
#endif
			...
			*/
			...
			if (dhd->info->power_state != PM_FAST) {
				power_mode = PM_FAST;
				ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_PM, (char *)&power_mode,
					sizeof(power_mode));
				if (ret >= 0)
					dhd->info->power_state = power_mode;
			}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && defined(CONFIG_PM_SLEEP)
	dhd->dhd_pm_notifier.notifier_call = dhd_sleep_pm_callback;
	dhd->dhd_pm_notifier.priority = 0;
	register_pm_notifier(&dhd->dhd_pm_notifier);
#endif /*  (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && defined(CONFIG_PM_SLEEP) */
...
dhd_write_macaddr(char *addr)
{
	...
	strncpy(filepath, "/data/calibration/mac_info", sizeof(filepath));
...
#ifdef WRITE_MACADDR
		/* Write MAC addr to an external file */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && defined(CONFIG_PM_SLEEP)
			unregister_pm_notifier(&dhd->dhd_pm_notifier);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && defined(CONFIG_PM_SLEEP) */
			free_netdev(ifp->net);
#ifdef CONFIG_HAS_WAKELOCK
...
{
	return platform_driver_unregister(&wifi_device);
}

void dhd_card_remove(void)
{
	DHD_TRACE(("%s: Enter\n", __FUNCTION__));
