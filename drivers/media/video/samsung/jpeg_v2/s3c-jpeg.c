		...
		unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_JPG_DECODE:
		...
		jpg_reg_ctx->jpg_data_addr = (unsigned int)jpg_data_base_addr
			+jpg_reg_ctx->bufinfo->main_stream_start;

		jpg_reg_ctx->img_data_addr =(unsigned int)jpg_data_base_addr
	...
	case IOCTL_JPG_ENCODE:
		...
		if (param.enc_param->enc_type == JPG_MAIN) {
			jpg_reg_ctx->jpg_data_addr =(unsigned int)jpg_data_base_addr+jpg_reg_ctx->bufinfo->main_stream_start;
			if(jpg_reg_ctx->use_external_mm){
				jpg_reg_ctx->img_data_addr = jpg_reg_ctx->use_external_mm;
			}else{
				jpg_reg_ctx->img_data_addr =(unsigned int)jpg_data_base_addr
					+ jpg_reg_ctx->bufinfo->main_frame_start;
			}
			printk("enc_img_data_addr=0x%08x,"
	...
	case IOCTL_JPG_GET_STRBUF:
		...
		out = copy_from_user(&param, (struct jpg_args *)arg,
				sizeof(struct jpg_args));
		param.phy_in_buf = (char*)(jpg_data_base_addr + jpg_reg_ctx->bufinfo->main_stream_start);
		out = copy_to_user((void*)arg,(void*)&param,sizeof(param));
		unlock_jpg_mutex();
		return (unsigned int)param.mapped_addr + jpg_reg_ctx->bufinfo->main_stream_start;
	...
	case IOCTL_JPG_SET_PHY_FRMBUF:		

		jpg_dbg("IOCTL_JPG_SET_PHY_FRMBUF\n");
		//jpg_reg_ctx->bufinfo->main_frame_start = arg;
		jpg_reg_ctx->use_external_mm = arg;
		unlock_jpg_mutex();
		return 0;

	case IOCTL_JPG_GET_BUF_INFO:

		out = copy_to_user((void *)arg, jpg_reg_ctx->bufinfo, sizeof(struct jpegv2_buf));
		unlock_jpg_mutex();
		return 0;

	case IOCTL_JPG_GET_LIMIT_INFO:

		out = copy_to_user((void *)arg, jpg_reg_ctx->limits, sizeof(struct jpegv2_limits));
		unlock_jpg_mutex();
		return 0;

	case IOCTL_JPG_INVALIDE_CACHE:

		out = copy_from_user(&param, (struct jpg_args *)arg,
				sizeof(struct jpg_args));
		virtaddr = phys_to_virt((unsigned long)param.phy_out_buf);
		dmac_flush_range(virtaddr,virtaddr + param.out_buf_size);
		jpg_dbg("IOCTL_JPG_INVALIDE_CACHE\n");
		break;

	case IOCTL_JPG_CLEAN_CACHE:

		out = copy_from_user(&param, (struct jpg_args *)arg,
				sizeof(struct jpg_args));
		virtaddr = phys_to_virt((unsigned long)param.phy_in_buf);
		dmac_map_area(virtaddr, param.in_buf_size,DMA_TO_DEVICE);
		jpg_dbg("IOCTL_JPG_CLEAN_CACHE\n");
		break;
...
void s3c_jpg_plat_init(struct s3c_platform_jpeg *pdata)
{
...
#if defined(CONFIG_MACH_MEIZU_M9W) 
	main_pixels = 0;
	thumb_pixels = 0;
#else
	...
	s3c_jpeg_bufinfo.main_stream_size = ALIGN(2048*SZ_1K, PAGE_SIZE);
	/* Assuming JPEG V2 uses YCBCR422 output format */
