...
static int s3c_csis_clk_on(struct platform_device *pdev)
{
	...
	/* clk_csis */
	s3c_csis->clk_csis = clk_get(&pdev->dev, "clk_csis");
	/* sclk_csis */
	...
	clk_set_parent(s3c_csis->clock, parent);

	clk_set_rate(s3c_csis->clock, pdata->clk_rate);

	/* Turn on csis power domain regulator */
	regulator_enable(s3c_csis->regulator);
	/*clock enable for csis*/
	clk_enable(s3c_csis->clk_csis);
