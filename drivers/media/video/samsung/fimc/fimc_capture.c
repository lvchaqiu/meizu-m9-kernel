...
int fimc_cal_camera_output_size(struct fimc_control *ctrl)
{
 ...
 */
static  int __maybe_unused fimc_camera_start(struct fimc_control *ctrl)
{
...
int fimc_s_fmt_vid_capture(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	*/
	...
	mbus_fmt.reserved[0] = f->fmt.pix.priv;
	/*
...
int fimc_streamon_capture(void *fh)
{
	...
	/*real start*/
	fimc_cal_camera_output_size(ctrl);
	pr_debug("%s: camera src width=%d, height=%d\n", __func__, ctrl->cam->width, ctrl->cam->height);
	pr_debug("%s: caputur dest width=%d, height=%d\n", __func__, ctrl->cap->fmt.width, ctrl->cap->fmt.height);
	pr_debug("%s: caputur fmt color space=%d, pixelformat=%d, field=%d\n", __func__, ctrl->cap->fmt.colorspace, ctrl->cap->fmt.pixelformat, ctrl->cap->fmt.field);

	if(cap->fmt.priv == V4L2_CAMERA_CAPTURE)/*2-capture picture mode*/
	{
		cap->skip_frame = 0;
	}else{
		cap->skip_frame = 2;
	...
	if (ctrl->cam->type == CAM_TYPE_MIPI && ctrl->id != 2) {
		subdev_call(ctrl, video, s_stream, 0);
		s3c_csis_start(ctrl->cam->mipi_lanes, ctrl->cam->mipi_settle, \
			ctrl->cam->mipi_align, ctrl->cam->width, ctrl->cam->height, ctrl->cam->pixelformat);
		subdev_call(ctrl, video, s_stream, 1);
	}
