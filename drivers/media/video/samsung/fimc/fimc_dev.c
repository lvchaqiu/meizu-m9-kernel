...
static int __devinit fimc_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(srclk)) {
		fimc_err("%s: failed to get source clock[%s] of fimc\n",
				__func__, pdata->srclk_name);
		goto err_v4l2;
	}
	...
	if (IS_ERR(ctrl->clk)) {
		fimc_err("%s: failed to get fimc clock source[%s]\n",
	...
	pr_info("FIMC%d registered successfully\n", ctrl->id);
