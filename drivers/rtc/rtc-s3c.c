 ...
 * RTC tick counter.  Convert the cycles elapsed since last read into
 * nsecs and adds to a monotonically increasing timespec.
 */
static struct timespec persistent_ts;
static cycles_t cycles, last_cycles;
static unsigned int persistent_mult, persistent_shift;
void read_hr_persistent_clock(struct timespec *ts)
{
	...
	last_cycles = cycles;
	cycles = s3c_rtc_base ? readw(s3c_rtc_base + S3C64XX_CURTICCNT) : 0;
	delta = (cycles - last_cycles) & CLOCKSOURCE_MASK(32);
...
static void __devinit init_read_persistent_clock(void)
{
	unsigned int tmp;
	...
	read_hr_persistent_clock(&persistent_ts);
}
#else
#define init_read_persistent_clock()	do { } while (0)
#endif
...
static int __devinit s3c_rtc_probe(struct platform_device *pdev)
{
	...
	do {
		unsigned int rtc_con = readw(s3c_rtc_base + S3C2410_RTCCON);
		rtc_con |= S3C2410_RTCCON_CLKOUTEN;
		rtc_con &= ~(0xF<<4);
		writew(rtc_con, s3c_rtc_base + S3C2410_RTCCON);
	} while (0);
#endif
