...
static int s3c_udc_probe(struct platform_device *pdev)
{
	...
	dev->udc_vcc_d = regulator_get(&pdev->dev, "vcc_usb");
	if (IS_ERR(dev->udc_vcc_d)) {
		printk(KERN_ERR "failed to find vcc_usb source\n");
		return -ENOENT;
	}
#else
