...
static void s3c_pm_save_uart(unsigned int uart, struct pm_uart_save *save)
{
	...
	save->uintmask = __raw_readl(regs + S3C2410_UINTMSK);
...
static void s3c_pm_restore_uart(unsigned int uart, struct pm_uart_save *save)
{
	...
	__raw_writel(save->uintmask, regs + S3C2410_UINTMSK);
...
static void s3c_pm_clear_uarts(void)
{
	...
	for (uart = 0; uart < CONFIG_SERIAL_SAMSUNG_UARTS; uart++) {
		...
		__raw_writel(0xf, regs+S5P_UINTP);
		__raw_writel(0xf, regs+S5P_UINTSP);
		__raw_writel(0xf, regs+S5P_UINTM);
	}
}
#else
static void s3c_pm_save_uarts(void) { }
...
#if defined(CONFIG_PM_TEST_SUSPEND) && defined(CONFIG_RTC_HR_READ_PERSISTENT_CLOCK)
