...
static struct regulator_consumer_supply s5pv210_pd_irom_supply[] = {
	REGULATOR_SUPPLY("pd", "s5p-irom"),
};
...
static struct regulator_consumer_supply s5pv210_pd_cam_supply[] = {
	...
	REGULATOR_SUPPLY("pd", "s5p-mipi-csis.0"),
...
static struct regulator_consumer_supply s5pv210_pd_lcd_supply[] = {
	...
	REGULATOR_SUPPLY("pd", "s5p-dsim"),
//	REGULATOR_SUPPLY("pd", "s5p-fimg2d"),
};
...
static struct regulator_init_data s5pv210_pd_irom_data = {
	.constraints = {
		.valid_modes_mask	= REGULATOR_MODE_NORMAL,
		.valid_ops_mask		= REGULATOR_CHANGE_STATUS,
	},
	.num_consumer_supplies	= ARRAY_SIZE(s5pv210_pd_irom_supply),
	.consumer_supplies	= s5pv210_pd_irom_supply,
};
...
static struct s5pv210_pd_config s5pv210_pd_irom_pdata = {
	.supply_name = "pd_irom_supply",
	.microvolts = 5000000,
	.init_data = &s5pv210_pd_irom_data,
	.ctrlbit = S5PV210_PD_IROM,
};
...
struct platform_device s5pv210_pd_irom = {
	.name          = "reg-s5pv210-pd",
	.id            = 6,
	.dev = {
		.platform_data = &s5pv210_pd_irom_pdata,
	},
};
...
static int __devinit reg_s5pv210_pd_probe(struct platform_device *pdev)
{
	...
	/* disable ip power when kernel starting */
#ifdef CONFIG_MACH_MEIZU_M9W
	if (s5pv210_pd_is_enabled(drvdata->dev))
		s5pv210_pd_disable(drvdata->dev);
#endif
