...
static int s5pv210_clk_gate1_ctrl(struct clk *clk, int enable)
{
...
static struct clk init_clocks_off[] = {
	{//IP0
		...
		.enable		= s5pv210_clk_ip3_ctrl,
	//	.ctrlbit	= (1 << 0),
		.ctrlbit	= S5P_CLKGATE_IP3_SPDIF | S5P_CLKGATE_IP3_PCM0 |
				  S5P_CLKGATE_IP3_PCM1 | S5P_CLKGATE_IP3_I2S0 |
				  S5P_CLKGATE_IP3_I2S1 | S5P_CLKGATE_IP3_I2S2,
	}, {
		.name		= "pcm",
		.id		= 2,
		.parent		= &clk_pclk_psys.clk,
...
void __init_or_cpufreq s5pv210_setup_clocks(void)
{
	...
	for (ptr = 0; ptr < ARRAY_SIZE(clksrcs); ptr++) {
		pclkSrc = &clksrcs[ptr];
		if (!strcmp(pclkSrc->clk.name, "sclk_mmc")) {
			clk_set_parent(&pclkSrc->clk, &clk_mout_mpll.clk);

			if (pclkSrc->clk.id == 0)
				clk_set_rate(&pclkSrc->clk, 42*MHZ);
			else
				clk_set_rate(&pclkSrc->clk, 25*MHZ);
		...
		} else if (!strcmp(pclkSrc->clk.name, "sclk_uart")) {
			clk_set_parent(&pclkSrc->clk, &clk_mout_mpll.clk);
			...
			clk_set_parent(&pclkSrc->clk, &clk_mout_mpll.clk);
			clk_set_rate(&pclkSrc->clk, 200000000);
		} else if (!strcmp(pclkSrc->clk.name, "sclk_fimd")) {
			clk_set_parent(&pclkSrc->clk, &clk_xusbxti);
		} else if (!strcmp(pclkSrc->clk.name, "sclk_mfc")) {
			clk_set_parent(&pclkSrc->clk, &clk_mout_mpll.clk);
		}
