...
static int m9w_codec_reset(struct platform_device *pdev)
{
	int ret = 0;

	switch (pdev->id) {
	case 0:
		ret = gpio_request(AUDIO_RESET, "m9w codec power");
		if (!ret) {
			gpio_set_value(AUDIO_RESET, 0);
			mdelay(10);
			gpio_set_value(AUDIO_RESET, 1);
			gpio_free(AUDIO_RESET);
		}
		break;
