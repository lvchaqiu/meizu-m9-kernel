		...
		int clk_id, unsigned int freq, int dir)
{
	...
	case S3C_CLKSRC_CLKAUDIO:
		...
		case 64000:
		case 96000:
			clk_set_rate(clk, 32768000);//49152000
			break;
		case 11025:
		case 22050:
		case 44100:
		case 88200:
		default:
			clk_set_rate(clk, 67737600);//67738000
			break;
		}
		clk_enable(clk);
		clk_put(clk);
		break;
#endif	/* CONFIG_MACH_MEIZU_M9W */
#endif	/* USE_CLKAUDIO */
	...
	case S5P_IISMOD_OPCLK_PCLK:
		iismod &= ~S5P_IISMOD_OPCLK_MASK;
		switch (dir) {
		case 0:
			iismod |= S5P_IISMOD_OPCLK_CDCLK_OUT;
			break;
		case 1:
			iismod |= S5P_IISMOD_OPCLK_CDCLK_IN;
			break;
		case 2:
			iismod |= S5P_IISMOD_OPCLK_BCLK_OUT;
			break;
		default:
			iismod |= S5P_IISMOD_OPCLK_PCLK;
			break;
		}
		break;

	/* IIS-IP is Slave, Select RCLK clock source as I2SCLK */
	case S3C64XX_IISMOD_SLAVE:
		iismod &= ~S3C64XX_IISMOD_IMSMASK;
		iismod |= clk_id;
		break;
#endif
...
{
	struct snd_soc_dai_ops *ops = (struct snd_soc_dai_ops *)dai->ops;
...
static __devinit int s3c64xx_iis_dev_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_MACH_MEIZU_M9W
	i2s->regulator_codec = regulator_get(NULL, "vcc_audio_1.8");
	if (IS_ERR(i2s->regulator_codec)) {
		dev_err(&pdev->dev, "%s: failed to get resource %s\n",
				__func__, "i2s");
	...
	/* Enable Power domain */
	regulator_enable(i2s->regulator_codec);

	if (i2s_pdata && i2s_pdata->cfg_gpio && i2s_pdata->codec_reset(pdev)) {
		dev_err(&pdev->dev, "Unable to reset codec\n");
		return -EINVAL;
	}
#endif
	/* Audio Clock
	...
	if (pdev->id == 0) {
		...
		do {
			struct clk *xxti;
			xxti = clk_get(NULL, "ext_xtal");
			if (IS_ERR(xxti)) {
				dev_err(&pdev->dev, "failed to get ext_xtal\n");
				clk_put(mout_audss);
				goto err1;
			}
			clk_set_parent(mout_audss, xxti);
			clk_put(xxti);
		} while(0);
#else
